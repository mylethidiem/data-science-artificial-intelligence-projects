...
- Device Address: The first byte (e.g., 1) specifies the address of the device on the Modbus network.
- Function Code: The second byte (e.g., 5) specifies the function code. In this case, 5 is the function code for writing a single coil (turning a relay on or off).
- Coil Address: The third and fourth bytes (e.g., 0, 0 or 0, 1 or 0, 2) specify the address of the coil (relay) to be controlled.
- Data: The fifth and sixth bytes (e.g., 0xFF, 0 or 0, 0) specify the data to be written to the coil. 0xFF, 0 typically means turning the relay on, and 0, 0 means turning it off.
- CRC: The last two bytes (e.g., 0x8C, 0x3A or 0xCD, 0xCA) are the Cyclic Redundancy Check (CRC) for error-checking.
"""
...
class ModbusMaster():
	...
	def __init__(self) -> None:
		post_list = list_ports.comports() # List available serial ports
		if len(port_list) == 0:
		...
		if which_os == "Linux":
			...
			postName = "/dev/" + name_ports[0] # Get the first USB port
	...
	def serial_read_data(self):
		...
		ser = self.ser
		bytesToRead = ser.inWaiting() # Get the number of bytes waiting in the serial buffer
		if bytesToRead > 0:
			print("Data received")
			data = ser.read(bytesToRead) # Read the bytes from the serial buffer
			data_array = [b for b in data] # Convert the bytes to a list
			print(data_array)
			if len(data_array) >= 7:
				array_size = len(data_array)
				value = data_array[array_size - 4] * 256 + data_array[array_size - 3] # Calculate the value from the data array
				return value
			else:
